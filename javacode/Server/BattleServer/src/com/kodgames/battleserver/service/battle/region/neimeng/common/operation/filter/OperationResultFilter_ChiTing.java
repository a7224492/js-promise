 ...
 * 吃牌上听操作过滤器
 */
public class OperationResultFilter_ChiTing extends OperationResultFilter
{
	@Override
	public boolean filter(BattleBean context, Step result, byte card, boolean phaseDeal)
	{
		Macro.AssetTrue(phaseDeal);
		Macro.AssetFalse(result.getPlayType() == PlayType.OPERATE_CAN_CHI_A_CARD);
		Macro.AssetFalse(result.getCards().size() == 3);
		...
		// 去掉要吃牌的手牌，检测剩余手牌
		List<Byte> chiHandCards = result.copy().getCards();
		chiHandCards.remove(0);
		chiHandCards.forEach(c -> handCards.remove(c));

		// 伪造吃牌Step
		Step chiStep = result.copy();
		chiStep.setPlayType(PlayType.OPERATE_CHI_A_CARD);
		cardHeap.add(chiStep);
		...
		if (canTing == true && context.getGameRules().contains(Rules_NeiMeng.TAN_BA_ZHANG))
		{
			...
			for (Map.Entry<CardType, Integer> map : typeCount.entrySet())
			{
				...
				// 若有某种花色的牌数量等于七张，则需要判断吃牌后可打出的牌是否包含这种花色，如果包含，则不能打出，否则胡牌时手牌无法满足摊八张的条件
				else if (map.getValue() == 7)
				{
					...
					// 若可打出的手牌全部为当前判断的花色，则不可吃，否则吃后上听，再打出一张牌后，便无法满足摊八张的条件
				...
				// 若此花色的牌数量小于七张，吃牌后无法满足摊八张条件，则无法上听
				else
					canTing = false;
			}
		}
