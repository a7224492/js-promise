package com.kodgames.battleserver.service.battle.region.meizhou.common.score.battle;
 ...
 * 梅州分数计算器
 */
public class BattleScoreCalculator_MeiZhou extends BattleScoreCalculator
{
	...
	private void calcHuScore(Collection<PlayerInfo> players, HashMap<Integer, PlayerMatchResultPROTO.Builder> inoutResultBuilders)
	{
		for (PlayerInfo player : players)
		{
			player.getCards().getScoreDatas().forEach((scoreData) -> {
				...
				ResultEventPROTO.Builder normalBuilder = calcNormalHuScore(player, scoreData, inoutResultBuilders, eventProto);
	 ...
	 */
	private ResultEventPROTO.Builder calcNormalHuScore(PlayerInfo playerInfo, ScoreData scoreData, HashMap<Integer, PlayerMatchResultPROTO.Builder> inoutResultBuilders, ResultEventPROTO.Builder eventProto)
	{
		// 设置加/减分数
		...
		mainScoreProto.setType(getMainHuType(scoreData, playerInfo));
	 ...
	 * 获取主胡牌类型
	 */
	private int getMainHuType(ScoreData data, PlayerInfo playerInfo)
	{
		return data.getSourceId() == playerInfo.getRoleId() ? PlayType.HU_ZI_MO : PlayType.HU_DIAN_PAO;
	}
