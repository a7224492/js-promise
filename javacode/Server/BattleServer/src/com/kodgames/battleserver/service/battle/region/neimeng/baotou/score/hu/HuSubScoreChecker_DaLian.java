 ...
 * 牌型算分：大连
 * 
 * 大连，2分。（胡牌时，组成胡牌牌型的两个同花色顺子以123456或456789的形式存在，如果只是包含这两个顺子，不算大连）。
 */
public class HuSubScoreChecker_DaLian extends HuSubScoreChecker
{
	public HuSubScoreChecker_DaLian()
	{
		super(PlayType.HU_DA_LIAN);
	}
	...
	public boolean calculate(BattleBean context, int roleId, HuScoreCheckContext inoutHuContext)
	{
		...
		// 如果任一花色的坎牌最小索引中同时有1和4，或4和7，即为大连
		boolean result = false;
		for (Set<Integer> cards : kanCards.values())
		{
			if ((cards.contains(0) && cards.contains(3)) || (cards.contains(3) && cards.contains(6)))
			{
