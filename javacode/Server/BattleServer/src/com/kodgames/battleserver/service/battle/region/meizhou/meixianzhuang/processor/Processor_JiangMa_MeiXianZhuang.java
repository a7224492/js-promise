...
public class Processor_JiangMa_MeiXianZhuang extends Processer
{
	/**
	 * 玩家对应的奖马信息数组
	 */
			...
			if (maGenGang)
			{
				...
				for (ScoreData scoreData : player.getCards().getScoreDatas())
				{
					if (PlayType.isGangOperator(scoreData.getPoints().get(0).getScoreType()))
					{
						...
						// {
						// // 杠牌分是直接计算，不会有最后结算，最后结算的时候只是算总分
						// scorePoint.setScoreValue(scorePoint.getScoreValue() * gangPoint.getScoreValue());
						// }
