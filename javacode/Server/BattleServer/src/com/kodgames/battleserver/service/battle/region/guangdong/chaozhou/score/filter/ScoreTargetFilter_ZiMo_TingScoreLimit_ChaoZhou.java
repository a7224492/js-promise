package com.kodgames.battleserver.service.battle.region.guangdong.chaozhou.score.filter;
...
/**
 * 10倍不计分，当有人自摸胡牌，其他三家如果存在所听牌型分不低于10倍底分的玩家时，则此人此局不付给胡牌者分数
 */
public class ScoreTargetFilter_ZiMo_TingScoreLimit_ChaoZhou extends ScoreTargetFilter
{
	...
	public boolean filter(BattleBean context, PlayerInfo player, PlayerInfo sourcePlayer, PlayerInfo checkingPlayer)
	{
		...
		for (byte tingCard : tingCards)
		{
			...
			// 潮州10倍不计分计算的就是倍数（所有分值也只有倍数（门清底分除外）），所以可以直接比较totalMulti
			maxScore = Math.max(maxScore, scoreCacluateContext.totalMulti);
		}
