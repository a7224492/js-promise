package com.kodgames.battleserver.service.battle.region.neimeng.baotou.creator;
...
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_DaLian;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_JiuLianBaoDeng;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_KanWuKui;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_Ke;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_LaoShao;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_SiGuiYi;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_XiaoLian;
import com.kodgames.battleserver.service.battle.region.neimeng.baotou.score.hu.HuSubScoreChecker_YiBanGao;
...
import com.kodgames.battleserver.service.battle.region.neimeng.common.score.battle.filter.BattleScoreFilter_Gang;
 ...
 * 内蒙：包头
 */
public class BattleCreator_NeiMeng_BaoTou implements IBattleCreator
{
	@Override
	...
	{
		return Rules_NeiMeng.checkRules(rules, Rules_NeiMeng.GAME_TYPE_BAOTOU);
	}
	...
	private void createHuCalculateProcessor(JSONObject context, List<Integer> rules)
	{
		...
		// 支持的分数
		{
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuMainScoreChecker_QiDui.class, ScoreCalculateType.TOTAL_ADD, 15));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuMainScoreChecker_ShiSanYao.class, ScoreCalculateType.TOTAL_ADD, 35));

			// 附加牌型：清一色，一条龙，豪华七对，一般高，够张，小连，大连，老少，壳
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_QingYiSe.class, ScoreCalculateType.TOTAL_ADD, 15, new int[] {PlayType.HU_GOU_ZHANG}));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_YiTiaoLong.class,
				ScoreCalculateType.TOTAL_ADD,
				15,
				new int[] {PlayType.HU_GOU_ZHANG, PlayType.HU_XIAO_LIAN, PlayType.HU_DA_LIAN, PlayType.HU_LAO_SHAO}));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_HaoHuaQiDui.class, ScoreCalculateType.TOTAL_ADD, 20, new int[] {PlayType.HU_QI_DUI}));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_YiBanGao.class, ScoreCalculateType.TOTAL_ADD, 1));
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_XiaoLian.class, ScoreCalculateType.TOTAL_ADD, 1));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_DaLian.class, ScoreCalculateType.TOTAL_ADD, 2));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_LaoShao.class, ScoreCalculateType.TOTAL_ADD, 1));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_Ke.class, ScoreCalculateType.TOTAL_ADD, 1));

			// 特殊加分项：单口（边张，坎张，单吊），四归一，砍五魁，缺门，连庄，自摸
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_SiGuiYi.class, ScoreCalculateType.TOTAL_ADD, 2));
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_KanWuKui.class, ScoreCalculateType.TOTAL_ADD, 5, new int[] {PlayType.HU_KAN_ZHANG, PlayType.HU_DAN_DIAO}));
			...
			// 特殊牌型：九莲宝灯
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_JiuLianBaoDeng.class,
				ScoreCalculateType.TOTAL_ADD,
				35,
				new int[] {PlayType.HU_PING_HU, PlayType.HU_QING_YI_SE, PlayType.HU_YI_TIAO_LONG, PlayType.HU_GOU_ZHANG, PlayType.HU_XIAO_LIAN, PlayType.HU_DA_LIAN, PlayType.HU_LAO_SHAO,
					PlayType.HU_KE, PlayType.HU_SI_GUI_YI, PlayType.HU_QUE_YI_MEN}));
	...
	private void createBattleScoreCalculator(JSONObject createContext, List<Integer> rules)
	{
		...
		int huScoreLimit = Rules_NeiMeng.getHuScoreLimit(rules);
		context.element(BattleScoreCalculator.KEY_MAX_VALUE, huScoreLimit);
	...
	private JSONObject createCalculate(List<Integer> rules)
	{
		...
		// 牌局相关分数
		{
			...
			// 牌局分数过滤
			{
				...
				// 未胡牌的人不能收杠分，胡牌人只向输牌人收杠分
				arrayContext.add(CreateContextHelper.createObject(BattleScoreFilter_Gang.class));
