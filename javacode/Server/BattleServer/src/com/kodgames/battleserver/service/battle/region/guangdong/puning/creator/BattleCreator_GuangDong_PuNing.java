package com.kodgames.battleserver.service.battle.region.guangdong.puning.creator;
...
import com.kodgames.battleserver.service.battle.region.guangdong.puning.score.BattleScoreCalculator_PuNing;
 ...
 * 潮汕规则的creator
 */
public class BattleCreator_GuangDong_PuNing implements IBattleCreator
{
	...
	{
		return Rules_GuangDong.checkRules(rules, Rules_GuangDong.GAME_TYPE_PU_NING);
	}
		...
		// 添加跟庄
		{
			if (rules.contains(Rules_GuangDong.SCORE_GEN_ZHUANG))
			{
				JSONArray afterFilters = CreateContextHelper.createArray();
				{
					...
					if (rules.contains(Rules_GuangDong.SCORE_GEN_ZHUANG_LIANG_FEN))
						afterProcess.element(AfterOperationProcessor_PlayCard_ChaoShan.KEY_SCORE_VALUE, -2);
	...
	{
		JSONObject context = CreateContextHelper.createObject(BattleScoreCalculator_PuNing.class);
			...
			|| rules.contains(Rules_GuangDong.PUNISH_HORSE_2))
		{
			...
			// 添加需要忽略的类型
			{
				...
				context.element(BattleScoreCalculator_PuNing.KEY_INGORE_TYPES, arrayContext);
			}
			// 添加马分的类型
			{
				JSONArray arrayContext = CreateContextHelper.createArray();
				arrayContext.add(isBuy ? PlayType.DISPLAY_BE_HU_BUY_HORSE_SCORE : PlayType.DISPLAY_BE_HU_PUNISH_HORSE_SCORE);
				arrayContext.add(isBuy ? PlayType.DISPLAY_HU_BUY_HORSE_SCORE : PlayType.DISPLAY_HU_PUNISH_HORSE_SCORE);
				context.element(BattleScoreCalculator_PuNing.KEY_BUY_MA_TYPES, arrayContext);
			}
		}
	...
	private void createHuCalculateProcessor(JSONObject context, List<Integer> rules)
	{
		...
		// 支持的分数
		{
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuMainScoreChecker_ShiSanYao.class, ScoreCalculateType.TOTAL_ADD, 26));
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_ShuangHaoHuaQiDui.class, ScoreCalculateType.TOTAL_ADD, 20, new int[] {PlayType.HU_QI_DUI,
				PlayType.HU_HAO_HUA_QI_DUI}));

			// 6. 三豪华七小对：7小对中，有3个4张牌一样 +30
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_SanHaoHuaQiDui.class, ScoreCalculateType.TOTAL_ADD, 30, new int[] {PlayType.HU_SHUANG_HAO_HUA_QI_DUI,
				PlayType.HU_QI_DUI, PlayType.HU_HAO_HUA_QI_DUI}));
			...
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_HunYaoJiu.class, ScoreCalculateType.TOTAL_ADD, 10, new int[] {PlayType.HU_PING_HU}));

			// 6、 清一九：整手牌由纯数字牌并且都是数字1和9组成，不包含字牌 +20
			arrayContext.add(CreateContextHelper.createHuScoreChecker(HuSubScoreChecker_QingYaoJiu.class, ScoreCalculateType.TOTAL_ADD, 20, new int[] {PlayType.HU_PING_HU}));
			...
			if (rules.contains(Rules_GuangDong.MASTER_CARD_FORE_HU))
			{
				if (rules.contains(Rules_GuangDong.MASTER_CARD_DOUBLE_SCORE))
					arrayContext.add(CreateContextHelper.createHuScoreChecker(HuMainScoreChecker_4MasterCard.class, ScoreCalculateType.TOTAL_ADD, 4, new int[] {PlayType.HU_PING_HU,
						...
						PlayType.HU_DA_SAN_YUAN, PlayType.HU_XIAO_SAN_YUAN, PlayType.HU_DA_SI_XI, PlayType.HU_XIAO_SI_XI, PlayType.HU_YI_TIAO_LONG, PlayType.HU_QING_YAO_JIU}));
				else
