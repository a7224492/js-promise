 ...
 * 检测牌型：坎张
 * 
 * 胡2张牌之间的牌。4556胡5也为坎张，手中有45567胡6不算坎张。1分。
 */
public class HuSubScoreChecker_KanZhang extends HuSubScoreChecker
{

	public HuSubScoreChecker_KanZhang()
	{
		super(PlayType.HU_KAN_ZHANG);
	}
	...
	public boolean check(BattleBean context, int roleId, HuScoreCheckContext inoutHuContext)
	{
		...
		Set<Byte> pattern = getPattern(card);
	...
	private Set<Byte> getPattern(byte card)
	{
		CardType type = CardType.getCardType(card);

		// 坎张模式
		byte[] pattern = {(byte)(card - 1), card, (byte)(card + 1)};
