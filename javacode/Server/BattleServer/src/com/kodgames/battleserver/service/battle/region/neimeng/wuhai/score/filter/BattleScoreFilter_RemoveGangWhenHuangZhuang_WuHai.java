 ...
 * 黄庄之后, 所有杠的分数不算
 */
public class BattleScoreFilter_RemoveGangWhenHuangZhuang_WuHai extends BattleScoreFilter
{
	public void filter(BattleBean context)
	{
		...
		context.getPlayers().values().forEach(player -> {
			...
			player.getCards().getScoreDatas().removeIf(score -> {
				if (score.getPoints().get(0).getScoreType() != PlayType.DISPLAY_PEI_GANG && PlayType.isGangOperator(score.getPoints().get(0).getScoreType()))
					return true;
				return false;
			});
		});
